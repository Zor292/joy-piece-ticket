...
async function handleCommand(message, client) {
  ...
  if (command === 'tickets-list') {
    if (!isAdmin(member)) return message.reply({ embeds: [embed(null, 'ليس لديك صلاحية.')] });
    if (openTickets.size === 0) return message.reply({ embeds: [embed('التذاكر المفتوحة', 'لا توجد تذاكر مفتوحة حالياً.')] });
    const list = [...openTickets.entries()].map(([cId, t]) => {
      const typeConfig = config.TICKET_TYPES[t.type];
      return `<#${cId}> | <@${t.ownerId}> | ${typeConfig?.label || t.type}`;
    }).join('\n');
    return message.reply({ embeds: [embed('التذاكر المفتوحة', list)] });
  }
    ...
    return message.reply({
      embeds: [new EmbedBuilder()
        ...
        .addFields(
          ...
          {
            name: 'الادارة',
            value: `\`!ban\` \`!unban\` \`!kick\` \`!mute\` \`!unmute\` \`!warn\` \`!clear\` \`!slowmode\` \`!lock\` \`!unlock\` \`!lockdown\` \`!unlockdown\` \`!nick\` \`!role-add\` \`!role-remove\``,
          },
          {
            name: 'المعلومات',
  ...
  if (command === 'stats') {
    ...
    return message.reply({
      embeds: [embed('احصائيات البوت', null, [
        ...
        { name: 'التذاكر المفتوحة', value: openTickets.size.toString(), inline: true },
        { name: 'البينج', value: `${client.ws.ping}ms`, inline: true },
      ])]
    });
  }
