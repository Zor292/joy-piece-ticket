...
client.on('interactionCreate', async (interaction) => {
  try {
    ...
    if (interaction.isStringSelectMenu()) {
      ...
      // Legacy dropdown support (kept for compatibility)
      ...
      // Extra tickets dropdown (أخرى)
      if (interaction.customId === 'open_ticket_type_extra') {
        const ticketType = interaction.values[0];
        await interaction.deferReply({ ephemeral: true });
        await createTicket(interaction, ticketType);
        return;
    ...
    if (interaction.isButton()) {
      ...
      // ── Components V2: Open ticket buttons ──
      if (customId.startsWith('open_ticket_btn_')) {
        const ticketType = customId.replace('open_ticket_btn_', '');
        await interaction.deferReply({ ephemeral: true });
        await createTicket(interaction, ticketType);
        return;
      }
      ...
      if (customId.startsWith('ticket_senior_')) {
        ...
        await interaction.reply({
          components: [{
            ...
            components: [{
              type: 10,
              content: `### تم استلام التكت\n<@${interaction.user.id}> استلم هذا التكت`,
            }],
          }],
          flags: 1 << 15,
        });
        ...
        // Confirmation with Components V2
        await interaction.reply({
          components: [{
            type: 17,
            ...
            components: [
              {
                type: 10,
                content: `### تأكيد الإغلاق\nهل أنت متأكد من إغلاق هذا التكت؟\nسيتم حذف القناة وإرسال السجل للوق.`,
              },
              {
                type: 1,
                components: [
                  { type: 2, custom_id: `ticket_confirm_close_${channelId}`, label: 'نعم، اغلق التكت', style: 4 },
                  { type: 2, custom_id: `ticket_cancel_close_${channelId}`, label: 'إلغاء', style: 2 },
                ],
              },
            ],
          }],
        ...
        try {
          ...
          await interaction.reply({
            components: [{
              type: 17,
              accent_color: 0x2ecc71,
              components: [{ type: 10, content: `تم اضافة ${targetMember} للتكت.` }],
            }],
            flags: 1 << 15,
          });
        } catch (e) {
