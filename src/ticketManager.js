...
const openTickets = new Map(); // channelId -> { type, ownerId, claimedBy, openedAt }
...
async function createTicket(interaction, ticketType) {
  ...
  ticketCounter.count++;
  const num = String(ticketCounter.count).padStart(4, '0');
  const typeShort = { tech: 'دعم', purchase: 'شراء', complaint: 'شكوى', compensation: 'تعويض' };
  const channelName = `${typeShort[ticketType]}-${num}-${user.username}`.toLowerCase().replace(/[^\u0600-\u06FFa-z0-9-]/g, '-').slice(0, 50);
...
async function closeTicket(interaction, channelId) {
  ...
  const htmlContent = generateTranscriptHTML(sorted, ticketData, channel.name, interaction.guild);
...
function generateTranscriptHTML(messages, ticketData, channelName, guild) {
  ...
  const rows = messages.map((msg) => {
    ...
    const attachments = msg.attachments.size > 0
      ? [...msg.attachments.values()].map(a => `<a href="${a.url}" target="_blank">[مرفق: ${a.name}]</a>`).join(' ')
      : '';
    return `
      <div class="message">
        <div class="meta">
...
<h1>سجل التكت</h1>
<div class="info">
  <span>القناة: <strong>${channelName}</strong></span>
  <span>صاحب التكت: <strong>${ticketData.ownerId}</strong></span>
  <span>نوع التكت: <strong>${typeConfig ? typeConfig.label : ticketData.type}</strong></span>
