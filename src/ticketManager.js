...
async function createTicket(interaction, ticketType) {
  ...
  const ruleLines = config.RULES_DESCRIPTION.split('\n\n');

  const ruleSections = ruleLines.map(block => {
    ...
    return {
      ...
      accessory: {
        type: 12, // Thumbnail
        media: { url: 'https://cdn.discordapp.com/emojis/1314577351660797993.png' },
      },
    };
  }).filter(s => {
    const content = s.components[0].content;
  ...
  const ticketContainer = {
    ...
    components: [
      ...
      {
        type: 11,
        items: [{ media: { url: config.IMAGE_URL } }],
      },
      // Title
      {
        type: 10,
        content: `## ${typeConfig.title}`,
      },
      { type: 14 },
      // Rules as plain text block
      {
        type: 10,
        content: `**قبل فتح اي تكت يرجى قراءة القوانين التالية والالتزام بها**\n\n${config.RULES_DESCRIPTION}`,
      },
      { type: 14 },
      // Action buttons row
      {
      ...
      {
        type: 10,
        content: `-# Developed by firas • JoyPiece`,
      },
    ],
  };
